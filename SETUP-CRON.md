...
> owner/repo: `scale-ag/dash-partiu-imersao-set26` · URL do Pages:
> `https://scale-ag.github.io/dash-partiu-imersao-set26/`. O **token
...
⚠️ **O Pages pode precisar ser ligado uma vez, na mão.** O workflow tenta ligá-lo
sozinho (`actions/configure-pages` com `enablement: true`), mas o `GITHUB_TOKEN`
do Actions pode não ter permissão para **criar** o site do Pages — o passo falharia
com `Create Pages site failed: Resource not accessible by integration`. Criar o
site exige permissão de **administração do repositório**, que o token do Actions
não tem por design.

Se isso acontecer, ligue uma vez em **Settings → Pages → Build and deployment →
Source: GitHub Actions** e re-execute o workflow. A partir daí todos os builds
seguintes publicam sozinhos.

URL pública após publicar: `https://scale-ag.github.io/dash-partiu-imersao-set26/`
...
- Repository access: **Only select repositories → `dash-partiu-imersao-set26`**
...
### URL
```
https://api.github.com/repos/scale-ag/dash-partiu-imersao-set26/actions/workflows/deploy.yml/dispatches
```
### Método
```
POST
